...
以下のルールを絶対に守って作成してください。

【厳守ルール】
1. **自然な一つの文章として構成する**
   - ユーザーの発言をただ羅列したりブツ切りで繋ぐのではなく、文脈が通じる自然な一つの日記文章にしてください。
   - 口語は日記らしい自然な口語調（「〜だな」「〜だった」など）に整えてください。
2. **ユーザーの言葉をそのまま活かす**
   - ユーザーが言ったニュアンスや表現はなるべくそのまま盛り込んでください。
3. **捏造の禁止（絶対遵守）**
   - **ユーザーが言っていない事実や気持ちを勝手に付け足すことは絶対しないでください**。推測による過度な脚色も禁止です。

以下のJSON形式のみを返してください：
{
  "title": "日記のタイトル（15文字以内）",
  "summary": "日記の本文（自然に繋がった文章。捏造は厳禁）",
