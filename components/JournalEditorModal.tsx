      ...
      >
        <View className="bg-white rounded-t-3xl h-[90%]">
          ...
          <ScrollView className="flex-1 px-6 pt-6" showsVerticalScrollIndicator={false}>
            ...
            <View className="mb-6">
              <View className="flex-row items-center mb-2">
                <Text className="text-indigo-600 mr-2">✏️</Text>
                <Text className="text-xs text-slate-500 font-bold uppercase tracking-wider">タイトル</Text>
              </View>
              <TextInput
                value={entry.title}
                onChangeText={(text) => setEntry(prev => prev ? {...prev, title: text} : null)}
                className="text-xl font-bold text-slate-800 bg-slate-50 rounded-xl px-4 py-3 border border-slate-200"
                placeholder="タイトルを入力"
            ...
            <View className="mb-6">
              <View className="flex-row items-center mb-2">
                ...
                <Text className="text-xs text-slate-500 font-bold uppercase tracking-wider">本文</Text>
              </View>
              <TextInput
                value={entry.summary}
                onChangeText={(text) => setEntry(prev => prev ? {...prev, summary: text} : null)}
                className="text-base text-slate-700 leading-7 bg-slate-50 rounded-xl px-4 py-4 min-h-[180px] border border-slate-200"
                multiline
            ...
            <View className="mb-8">
              <View className="flex-row items-center mb-3">
                <Text className="text-indigo-600 mr-2">💭</Text>
                <Text className="text-xs text-slate-500 font-bold uppercase tracking-wider">今日の気分</Text>
              </View>
                ...
                {[
                  ...
                  >
                    <Text className={`text-sm ${
                      entry.emotion === emotion.value ? 'text-white' : 'text-slate-600'
                    }`}>
                      {emotion.label}
                    </Text>
                  </TouchableOpacity>
          ...
          <View className="px-6 pb-8 pt-4 bg-white border-t border-slate-100">
            ...
            <TouchableOpacity 
              onPress={onCancel}
              className="bg-slate-100 rounded-2xl py-4"
            >
              <Text className="text-slate-500 font-medium text-center">キャンセル</Text>
            </TouchableOpacity>
          </View>
        </View>
      </KeyboardAvoidingView>
